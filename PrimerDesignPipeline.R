...
query=paste0("$query = '\"", seqRequest,"\"[organism] NOT Bacteria[organism] AND mitochondrion[filter] AND (\"",gene,"\")';")
...
specificity=.95 #the proportion of the target sequence records that must be good primer matches
...
primerResults<-primerResults[primerResults$Tm_primer_1>50&primerResults$Tm_primer_1<60,]  #filter out if Tm greater than 50 or less than 40
primerResults<-primerResults[primerResults$Tm_primer_2>50&primerResults$Tm_primer_2<60,]  #filter out if Tm greater than 50 or less than 40
